...
# 치과 문서 → 1:1 카드뉴스 인포그래픽 변환 스킬

## 개요

워드 문서를 분석하여 **800×800px 1:1 비율**의 카드뉴스 스타일 HTML 인포그래픽을 **장마다 별도 파일**로 생성합니다.
핵심은 **페이지 기획안을 먼저 사용자에게 확인받고** 최종 제작하는 2단계 워크플로우입니다.
...
```css
--primary:      #0EA5E9;   /* 메인 파랑 */
--primary-dark: #0369A1;   /* 진한 파랑 */
--accent:       #10B981;   /* 민트 그린 */
--accent-warm:  #F97316;   /* 따뜻한 포인트 */
--bg:           #F0F9FF;   /* 연한 배경 */
--text:         #0F172A;   /* 본문 */
--text-light:   #64748B;   /* 보조 */
```
...
모든 카드 HTML을 **2배 해상도(1600×1600px)**로 변환하는 Python 스크립트를 실행하세요.
`device_scale_factor=2`를 사용하면 CSS 레이아웃은 800×800 그대로 유지하면서 실제 픽셀은 2배로 렌더링되어 텍스트와 그래픽이 선명합니다.
...
async def convert_all():
    html_files = sorted(glob.glob("/home/claude/card_*.html"))
...
```bash
cp /home/claude/card_*.png /mnt/user-data/outputs/
```

`present_files`로 **PNG 파일만** 전달하세요. HTML 중간 파일은 전달하지 않습니다.
...
docx 읽기
    ↓
기획안 제시 (3~5장) → 💬 사용자 확인 대기
    ↓ (승인 후)
각 장 HTML 생성 (800×800px, 별도 파일)
    ↓
Playwright로 HTML → PNG 변환 (2배 해상도 1600×1600px)
    ↓
PNG 파일만 present_files로 전달
```
...
- [ ] 우하단 장 번호 표시 (N/전체)
- [ ] **최종 출력은 2배 해상도 PNG 파일 (1600×1600px, HTML은 중간 파일로만 사용)**
- [ ] PNG 변환 시 폰트 로딩 완료 후 캡처 확인
